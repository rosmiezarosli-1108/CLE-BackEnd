...
public class AleContainerService : IAleContainerService
{
    ...
    public async Task<IEnumerable<AleContainerDto>> GetAllAleContainersByBookingAgent(string bookingAgentId)
    {
        var aleContainers = await _dbContext.AleContainers
            .Include(c => c.ToAddress)
            .Include(c => c.UpdateHistory)
    ...
    public async Task<IEnumerable<AleContainerDto>> GetAllAleContainersByConsignee(string consigneeId)
    {
        var aleContainers = await _dbContext.AleContainers
            .Include(c => c.ToAddress)
            .Include(c => c.UpdateHistory)
